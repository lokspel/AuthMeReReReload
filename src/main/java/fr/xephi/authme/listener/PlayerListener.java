...
public class PlayerListener implements Listener {
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        ...
        if (!PlayerListener19Spigot.isPlayerSpawnLocationEventCalled()
            && !PlayerListener1219.isAsyncPlayerSpawnLocationEventCalled()) {
            teleportationService.teleportOnJoin(player);
        }

        quickCommandsProtectionManager.processJoin(player);

        // Delay join processing to allow ProtocolLib to capture client locale from SETTINGS packet
        // This ensures i18n messages are sent in the correct language from the first message
